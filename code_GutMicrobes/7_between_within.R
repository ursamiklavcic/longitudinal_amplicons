# Between and within inddividual distances: 
...
wilcox_within <- pairwise.wilcox.test(bray_within$median_value, bray_within$fraction, paired = TRUE)
wilcox_within
...
wilcox_between <- pairwise.wilcox.test(bray_between$median_value, bray_between$fraction, paired = TRUE)
wilcox_between
...
# Linear mixed model per person for time 
fit_time <- lmer(median_value ~ date_dist * is_ethanol_resistant * taxonomy + (1 | person.x) + (1 | person.y), 
                 data = bray_within)
summary(fit_time)
...
bray_plots <- ggarrange(bray_boxplot + labs(tag = 'A'), 
          b_time + labs(tag = 'B'), common.legend = TRUE, legend = 'bottom', ncol=2, widths = c(0.8, 1))
bray_plots

ggsave('out/figures/bray_curtis_within_between.png', dpi = 600)

ggarrange(bray_plots, persistence_plot +labs(tag = 'C'), nrow = 2, 
...
difference <- bray_within %>%
  ...
  mutate(Bacillota = abs(`Ethanol-resistant Bacillota`- `Non ethanol-resistant Bacillota`), 
         Other =  abs(`Other ethanol-resistant taxa`- `Other non ethanol-resistant taxa`))
