...
# event data
event_data <- read.table('data/extreme_event_data.csv', sep=';', header = T)
...
ggplot(alpha_meta, aes(x=day, y=evenness)) +
  ...
  geom_line(data=alpha_meta %>% dplyr::select(-person) %>% filter(biota == 'untreated sample'), 
  ...
  geom_line(data=alpha_meta %>% filter(biota == 'untreated sample'),
...
evennessEM <- alpha_meta %>% select(original_sample, biota, evenness, person) %>%
  ...
  ggplot(aes(x =`untreated sample`, y= `ethanol treated sample` )) +
  geom_point() +
