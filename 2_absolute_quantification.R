...
# 
samples_info = read_excel('data/vzorci.xlsx', sheet = 3)

ddPCR_microbiota = read.quantasoft('data/absolute_quantification/20240322_plate1_updated_results.csv') %>%
  mutate(plate = 1) %>%
  ...
  rbind(read.quantasoft('data/absolute_quantification/20240327_plate3_update_results_repeats.csv') %>%
          mutate(plate = 3)) %>%
  left_join(samples_info, by ='Group') %>%
  # Calculate the copy number of 16s rRNA gene per ng of DNA
  mutate(CopiesPerngDNA = (Concentration*10*10E6)/DNAconc_seq,
         CopiesPerulSample = CopiesPerngDNA * DNAconc)
...
otutab_abs_rel = rownames_to_column(as.data.frame(otutabEM), 'Group') %>% 
  pivot_longer(cols = starts_with('Otu')) %>%
  group_by(Group) %>%
  mutate(rel_abund = value/sum(value)) %>%
  ungroup() %>%
  left_join(ddPCR_microbiota, by = join_by('Group' == 'Sample'), relationship = 'many-to-many') %>%
  mutate(abs_abund_ng = rel_abund*CopiesPerngDNA, 
         abs_abund_ul = rel_abund*CopiesPerulSample) %>%
  filter(!is.na(Well))

otutab_abs_rel_plot= otutab_abs_rel %>% left_join(taxtab, by = 'name') %>%
...
# library(ggbeeswarm) 
# otutab_abs_rel_plot %>% ggplot(aes(x=taxon, y=value, color=name)) +
#   geom_beeswarm() +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
